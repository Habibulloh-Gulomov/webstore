			...
			<div className="flex space-x-4 overflow-x-auto scrollbar-hide">
				{categories.map((category, index) => (
					...
					href='/category'
						...
						<p className="flex-1 mt-1 md:pl-1 h-max text-xs line-clamp-1 break-all sm:text-sm md:text-base font-semibold">{category.name}</p>
						<img src={category.photo} alt="" className="h-14 w-14 sm:h-16 sm:w-16 object-contain md:h-20 md:w-20" />
					</Link>
				))}
			</div>
		</div>
