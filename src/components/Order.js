    ...
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get(
          ...
          {
            headers: {  token: "eyJhbGciOiJIUzI1NiJ9.MQ.1tRPowLx-U0FAddHad0zerSPN41lydQhy7A-toLHzBo", },
          }
        );
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.order_id} className="border-t border-gray-200 hover:bg-gray-50">
                <td className="py-3 px-4">{order.paymart_client.contract_id}</td>
                <td className="py-3 px-4"> <a href={order.client_act_pdf}>Shartnoma</a></td>
                ...
                <td className="py-3 px-4">{order.paymart_client.price_month}</td>
                <td className="py-3 px-4 text-center flex">
                  <button
                    onClick={() => handleCheck(order.order_id)}
                  ...
                  <button
                    onClick={() => handleDelete(order.paymart_client.contract_id)}
